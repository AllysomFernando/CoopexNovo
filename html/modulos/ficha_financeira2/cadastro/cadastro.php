...
//session_start();
	...
	require_once("modulos/ficha_financeira2/funcoes_sagres.php");
	...
	$_SESSION['ficha_financeira']['carga_horaria_personalizada']				= 0;
	...
	if(isset($dados->total_diciplinas_pacote)){// add isset para verificar se a variavel dados foi criada.
		$_SESSION['ficha_financeira']['carga_horaria_pacote'] = $dados->total_diciplinas_pacote; 
	}
...
<main id="js-page-content" role="main" class="page-content">
	...
	<form class="needs-validation" novalidate="" method="post" target="dados" action="modulos/ficha_financeira2/cadastro/cadastro_dados.php">
										...
										<table class="table table-bordered table-hover table-striped w-100">
											...
											<tfoot>
												<tr>
													...
													<td class="text-center">
														<strong style="display: none" id="valor_semestre">R$ <?=isset($_GET['id']) ? number_format($_SESSION['ficha_financeira']['valor_semestre'], 2, ',', '.') : "0,00"?></strong>
														...
														<?		
																$sql2 = "SELECT
																			id_valor_hora, valor_mensalidade
																		FROM
														...
														?>	
															<option <?=$dados->valor_mensalidade == $valor->valor_mensalidade ? 'selected=""' : ''?> value="<?=$valor->id_valor_hora?>">R$ <?= number_format($valor->valor_mensalidade, 2, ',', '.');?></option>
														<?
																}
														?>	
														</select>
										...
										<table id="diciplinas_pacote" class="table table-bordered table-hover table-striped w-100">
											...
											<tfoot>
												<tr style="background-color: #ddd">
													...
													<td class="text-center">
														<!-- add isset na variavel personalizado para corrigir erro de variavel não definida na pagina -->
														<input onkeyup="definir_carga_horaria_pacote()" class="form-control text-center" <?= isset($_GET) && isset($personalizado) ? "" : "readonly"?>  type="text" name="total_diciplinas_pacote" id="total_diciplinas_pacote" value="<?= isset($_GET) && isset($personalizado) ? $dados->total_diciplinas_pacote : 0?>">
														<!-- <strong id="total_diciplinas_pacote">0</strong> -->
													</td>
			  ...
			  <div id="panel-2" class="panel">
				<div class="panel-hdr">
												...
												<tr id="grade_montada_<?=$disciplina->atc_id_atividade?>">
													...
													<td class="text-center">
														<?
															if($row->dp){
														?>
														<input onchange="definir_desconto_dp('<?=$disciplina->atc_id_atividade?>')" id="desconto_dp<?=$disciplina->atc_id_atividade?>" value="<?=intval($row->valor_desconto)?>" title="<?=$disciplina->atc_cd_atividade?>"  class="form-control">
														<?			
															} else {
														?>		
					...
					<div class="panel-container show">
						...
						<?
							if(isset($_GET['id']) && ($_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 10 || $_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 12 || $_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 1 || $_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 13)){
						?>
							<button type="button" onclick="alterar_link_whatsapp()" class="btn btn-default" data-toggle="modal" data-target="#default-example-modal-lg-center">Notificar Acadêmico</button>
							<?
								if(isset($_GET['id'])){
							?>	
							<button type="button" onclick="clonar()" class="btn btn-danger ml-4">Clonar Ficha Financeira</button>
							<?
								}
							?>
						<?
							...
							if(isset($_GET['id']) && ($_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 3)){
								if($enviado_tesouraria){
							
						?>
							<button type="button" class="btn btn-default" data-toggle="modal" data-target="#default-example-modal-lg-center">Notificar Acadêmico</button>
							<button type="button" class="btn btn-success ml-4" onclick="finalizar_ficha_financeira()">Finalizar Ficha Financeira</button>
							<button type="button" class="btn btn-info ml-4" onclick="encaminhar_cres()">Encaminhar para o CRES/FIES</button>
							<button type="button" class="btn btn-warning ml-4" onclick="aguardando_dp()">Aguardando desconto de DPS</button>
						<?
								}
							}
	
							if($_SESSION['coopex']['usuario']['sistema']['id_tipo_usuario'] == 13){
						?>	
							<div class="custom-control custom-checkbox">
								<button onclick="enviar_para_tesouraria()" class="btn btn-success ml-auto" type="text">Enviar para a Tesouraria</button>
							</div>
							<div class="custom-control">
								<button onclick="enviar_para_coordenacao()" class="btn btn-danger ml-auto" type="text">Devolver para a Coordenação</button>
							</div>
							<textarea placeholder="Mensagem para a Coordenação" id="obs" class="form-control ml-4"></textarea>
						<?
							}
						?>
...
<script>
	...
	function alterar_link_whatsapp(id_registro = ''){
		...
		texto += "https://coopex.fag.edu.br/ficha_financeira2/aprovacao/"+id_registro;
	...
	function carrega_grade(id_grade = ''){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_grade.php", {id_curso: $("#id_curso").val()})
		.done(function(json){
			$("#id_grade").empty();
	...
	function carrega_turma(id_turma){
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_turma.php", {id_semestre: $("#id_semestre").val(), id_curso: $("#id_curso").val()})
		.done(function(json){
			$.each( json, function( i, item ) {
	...
	function carrega_semestre(id_semestre = ''){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_semestre.php", {id_grade: id_grade})
		.done(function(json){
			$("#id_semestre").empty();
	...
	function carrega_disciplina_equivalente(){
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_disciplina_geral.php", {id_curso: $("#id_curso").val()})
		.done(function(json){
	...
	function ativar_select2(){
		...
		$(".js-consultar-disciplina-equivalente").select2({
			ajax:{
				url: "modulos/ficha_financeira2/cadastro/ajax/carrega_disciplina_equivalente.php",
				dataType: 'json',
				delay: 250,
	...
	function definir_equivalencia(id_disciplina, id_equivalente, ch, id_unidade_responsavel){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/definir_equivalencia.php", {id_disciplina: id_disciplina, id_equivalente: id_equivalente, ch: ch, id_unidade_responsavel, id_unidade_responsavel})
		.done(function(json){
			if(json){
	...
	function definir_valor_mensalidade_fisioterapia(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/definir_valor_mensalidade_fisioterapia.php", {valor_desconto: valor_desconto})
		.done(function(json){
			if(json){
				$("#valor_hora").text("R$ "+json.valor);
				$("#valor_mensalidade").text("R$ "+json.mensalidade);
				$("#valor_semestre").text("R$ "+json.valor_mensalidade);
				carrega_valor_ficha();
			}
		})
		.fail(function(jqxhr, textStatus, error) {
	...
	function definir_desconto_dp(id_disciplina){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/definir_desconto_dp.php", {id_disciplina: id_disciplina, valor_desconto: valor_desconto})
		.done(function(json){
			carrega_valor_ficha();
		})
		.fail(function(jqxhr, textStatus, error) {
	...
	function definir_turma(id_disciplina, id_classe = ''){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/definir_turma.php", {id_disciplina: id_disciplina, id_classe: id_classe})
		.done(function(json){
	...
	function definir_carga_horaria_pacote(){

		
		total_horas = $("#total_diciplinas_pacote").val();
	

		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/definir_carga_horaria_pacote.php", {total_horas: total_horas})
		.done(function(json){
			carrega_valor_ficha();
		})
		.fail(function(jqxhr, textStatus, error) {
	...
	function tornar_dp(id_disciplina){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/tornar_dp.php", {id_disciplina: id_disciplina, id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
		.done(function(json){
	...
	function clonar(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/clonar.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
		.done(function(json){
			...
			Swal.fire({
				...
				timer: 2000,
				onClose: () => {
					 window.location.href = "/ficha_financeira2/cadastro/"+json;
				}
			});
	...
	$(document).ready(function(){
		...
		<?	
			if(isset($_GET['id']) && $dados->total_diciplinas_pacote){
		?>		
				definir_carga_horaria_pacote();
		<?
			}	
		?>
		...
		$("#id_turma").change(function() {
			...
			if(id_turma == 1){
				$("#total_diciplinas_pacote").removeAttr("readonly");
				$("#total_diciplinas_pacote").val(<?=isset($_GET['id']) && $dados->total_diciplinas_pacote ? $dados->total_diciplinas_pacote : 0?>);
				$("#diciplinas_pacote thead").hide();
				definir_carga_horaria_pacote()
			} else {
				$("#total_diciplinas_pacote").attr('readonly', true);
				$("#diciplinas_pacote thead").show();
			}
		});
		...
		$(".js-consultar-disciplina").select2({
			ajax:{
				url: "modulos/ficha_financeira2/cadastro/ajax/carrega_disciplina_geral.php",
				dataType: 'json',
				delay: 250,
				data: function(params){
					return {
	...
	function carrega_pacote(id_turma = ''){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_pacote.php", {id_semestre: id_semestre, id_pessoa: id_pessoa, id_periodo: id_periodo, id_curso: id_curso, id_turma: id_turma, id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
		.done(function(json){
			...
			if($("#id_turma").val() != 1){
				//COMENTEI ESSA LINHA PARA RESOLVER UM PROBLEMA, PROVAVELMENTE VAI CAUSAR OUTRO
				//$("#total_diciplinas_pacote").val(ch_total);
			}
	...
	function carrega_dp(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_dp.php", {id_semestre: id_semestre, id_pessoa: id_pessoa, id_periodo: id_periodo, id_curso: id_curso})
		.done(function(json){
	...
	function horario(id_disciplina, id_classe){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/consulta_horario.php", {id_semestre: id_semestre, id_disciplina: id_disciplina, id_classe: id_classe})
		.done(function(json){
			console.dir(json);
	...
	function incluir_disciplina(id){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_pacote_inclusao.php", {id_semestre: id_semestre, id_pessoa: id_pessoa, id_periodo: id_periodo, id_disciplina: id, id_curso: id_curso, id_turma: id_turma})
		.done(function(json){
	...
	function incluir_disciplina_fora(id){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_fora_pacote_inclusao.php", {id_disciplina: id, id_pessoa: id_pessoa, id_curso: id_curso, id_semestre: id_semestre})
		.done(function(json){

			$.each( json, function( i, item ) {

				equivalente = '<select title="'+item.atc_id_atividade+'" data-placeholder="Selecione a disciplina..." class="js-consultar-disciplina-equivalente form-control">';
	...
	function remover_disciplina_pacote(id){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_pacote_remocao.php", {id_semestre: id_semestre, id_pessoa: id_pessoa, id_periodo: id_periodo, id_disciplina: id, id_curso: id_curso, id_turma: id_turma})
		.done(function(json){
	...
	function remover_disciplina_fora_pacote(id){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/disciplina_fora_pacote_remocao.php", {id_semestre: id_semestre, id_pessoa: id_pessoa, id_periodo: id_periodo, id_disciplina: id, id_curso: id_curso})
		.done(function(json){
			$.each( json, function( i, item ) {
				console.log(item);
				carrega_valor_ficha();
	...
	function carrega_dados(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_dados.php", {})
		.done(function(json){
			$.each( json, function( i, item ) {
	...
	function carrega_valor_ficha(){
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/carrega_valor_ficha.php", {})
		.done(function(json){
			$.each( json, function( i, item ) {
				...
				/*if($("#id_turma").val() == 1){
					$("#ch_total_ficha").html($("#total_diciplinas_pacote").val());
				} else {*/
	...
	function enviar_para_tesouraria(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/enviar_para_tesouraria.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, nome: nome})
			.done(function(json){
				Swal.fire({
					type: "success",
	...
	function enviar_para_coordenacao(){
		var obs = $("#obs").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/enviar_para_coordenacao.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, obs: obs})
			.done(function(json){
				Swal.fire({
					type: "success",
	...
	function notificar_aluno_tesouraria(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/notificar_aluno_tesouraria.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, whats:whats})
		.done(function(json){
	...
	function notificar_aluno_coordenacao(){
		var whats = $("#whatsapp").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/notificar_aluno_coordenacao.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, whats:whats})
		.done(function(json){
			window.history.back();
		})
		.fail(function(jqxhr, textStatus, error) {
	...
	function notificar_aluno_tesouraria_email(){
		...
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/notificar_aluno_tesouraria_email.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, email: contato_email})
			.done(function(json){
				$('#default-example-modal-lg-center').modal('toggle');
				Swal.fire({
					type: "success",
	...
	function finalizar_ficha_financeira(){
		var email = $("#email").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/finalizar_ficha_financeira.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
			.done(function(json){
				Swal.fire({
					type: "success",
	...
	function encaminhar_cres(){
		var email = $("#email").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/encaminhar_cres.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
			.done(function(json){
				Swal.fire({
					type: "success",
	...
	function aguardando_dp(){
		var email = $("#email").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/aguardando_dp.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>})
			.done(function(json){
				Swal.fire({
					type: "success",
					title: "Status definido com sucesso",
	...
	function notificar_aluno_coordenacao_email(){
		var email = $("#email").val();
		$.getJSON("modulos/ficha_financeira2/cadastro/ajax/notificar_aluno_coordenacao_email.php", {id_ficha_financeira: <?=isset($_GET['id']) ? $_GET['id'] : 0?>, email: email})
			.done(function(json){
				$('#default-example-modal-lg-center').modal('toggle');
				Swal.fire({
					type: "success",
