<?php
	...
	$sql = "SELECT valor_1 FROM coopex_reoferta.carga_horaria WHERE carga_horaria = $reoferta->carga_horaria";
	$ch = $coopex->query($sql);
							...
							<tbody>
								...
								<tr>
									...
									<?php
										if(@$row2->data_matricula){
									?>	
										<!-- <span data-toggle="tooltip" data-placement="top" title="Pago: R$ <?=$row2->valor_pago?> em <?=$row2->data_pagamento?>" data-original-title="Pago: R$ <?=$row2->valor_pago?> em <?=$row2->data_pagamento?>" onclick="matricula_funcoes(<?php echo $row2->id_matricula?>, '<?php echo $row2->data_vencimento_matricula?>')" <?php echo $link?> class="btn btn-<?php echo $cor_matricula;?> btn-sm btn-block waves-effect waves-themed"><?php echo $situacao_matricula;?></span> -->
