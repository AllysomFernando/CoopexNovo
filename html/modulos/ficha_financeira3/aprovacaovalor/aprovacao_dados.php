...
<?php
	...
	if($_POST){
		...
		$sql2 = "SELECT
					...
					WHERE
						id_pessoa = $ficha->id_pessoa";
		$res2 = mssql_query($sql2);
		$row2 = mssql_fetch_object($res2);

		if($id_etapa == 8){

			$sql = "UPDATE ficha_financeira.ficha_financeira SET id_etapa=8 WHERE id_ficha_financeira=$id_ficha_financeira";
			$coopex->query($sql);

			$remetente		= $ficha->email;
			$destinatario 	= $row2->id_faculdade == 1000000002
								? "fichafinanceira@fag.edu.br"
								: "tesourariatoledo@fag.edu.br";
								
			$nome			= $ficha->nome;

			$assunto		= "Ficha Financeira - Aprovada: $id_ficha_financeira";
		...
		if($id_etapa == 9){
			...
			$sql = "UPDATE ficha_financeira.ficha_financeira SET id_etapa=9 WHERE id_ficha_financeira=$id_ficha_financeira";
			$coopex->query($sql);

			$remetente		= $ficha->email;
			$destinatario 	= $row2->id_faculdade == 1000000002 ? "fichafinanceira@fag.edu.br" : "tesourariatoledo@fag.edu.br";
			$nome			= $ficha->nome;
