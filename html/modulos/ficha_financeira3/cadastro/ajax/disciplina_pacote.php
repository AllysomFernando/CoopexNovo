...
if ($id_turma) {
  $sql = "SELECT TOP
					1 pac_id_pacote, alu_nu_matricula, rca_ch_matutino, rca_ch_vespertino, rca_ch_noturno, pac_ds_pacote, rcr_id_curriculo 
				FROM
...
$_SESSION['ficha_financeira']['id_curriculo'] = $row['rcr_id_curriculo'];
...
$sql = "SELECT
			...
			WHERE id_ficha_financeira = " . $_GET['id_ficha_financeira'];

$res_valor = $coopex->query($sql);


if ($res_valor->rowCount()) {
  $row_valor = $res_valor->fetch(PDO::FETCH_OBJ);
  if ($row_valor->valor_mensalidade) {
    $_SESSION['ficha_financeira']['valor_semestre']   = $row_valor->valor_mensalidade;
  } else {
    ...
    if ($row['atc_id_unidade_responsavel'] == 1000000080) {
      ...
      $sql5 = "SELECT
						...
						FROM
							...
							INNER JOIN academico..CRS_curso ON crs_id_curso = rca_id_curso
							INNER JOIN academico..alu_aluno ON alu_id_pessoa = rca_id_aluno
							INNER JOIN academico..RCR_registro_curriculo ON rca_id_registro_curso = rcr_id_registro_curso
							INNER JOIN academico..HIS_historico_ingresso_saida ON rca_id_registro_curso = his_id_registro_curso 
						WHERE
							alu_id_pessoa = $id_pessoa 
							AND rcr_st_registro_saida IS NULL 
							AND crs_id_curso = $id_curso 
							AND his_id_forma_saida IS NULL";
      $res5 = mssql_query($sql5);
      $row5 = mssql_fetch_assoc($res5);

      $rcr_id_curriculo = $row5['rcr_id_curriculo'];
