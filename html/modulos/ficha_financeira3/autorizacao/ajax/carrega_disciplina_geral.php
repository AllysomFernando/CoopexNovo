<?php session_start();
	...
	$sql = "SELECT DISTINCT
			...
			WHERE
				eqv_id_atividade_equivalente IN ( SELECT atc_id_atividade FROM academico..ATC_atividade_curricular, academico..CLA_CLASSE WHERE atc_id_atividade = cla_id_atividade_curricular AND atc_id_unidade_responsavel = 1000000080 AND cla_id_periodo_letivo = $id_periodo_letivo ) 
				AND ( atc_nm_atividade LIKE '%$nome%' OR atc_cd_atividade LIKE '%$nome%' ) 
			ORDER BY
				atc_cd_atividade";
	$res = mssql_query($sql);
