...
$sql = "SELECT id_departamento FROM coopex.departamento_pessoa WHERE id_pessoa = $id_pessoa";
$res = $coopex->query($sql);
$cursos = [];
while($row = $res->fetch(PDO::FETCH_OBJ)){
	$cursos[] = $row->id_departamento;
}
...
$sql = "SELECT
		...
		FROM
			xp.v_cadastro a
		INNER JOIN xp.cursos b ON a.cursoPretendido = b.curso 
		WHERE
			id_curso IN ($cursos)";
						...
						<table id="dt-basic-example" class="table table-bordered table-hover table-striped w-100">
							<thead>
								<tr>
									...
									<th width="10">Cortesia</th>
									<th width="10">Inscrito no Vestibular</th>
							</thead>
							<tbody>
								<?
								$i = 0;
								...
								$sql = "SELECT
										...
										FROM
											xp.v_cadastro a
											INNER JOIN xp.cursos b ON a.cursoPretendido = b.curso 
											LEFT JOIN vestibular.pessoa c ON a.email = c.us_email
											LEFT JOIN vestibular.cortesia d ON a.id_cadastro = d.id_vestibular
										WHERE
											id_curso IN ( $cursos ) 
											group by email
										ORDER BY
											cursoPretendido, nome";
								$res = $coopex_antigo->query($sql);
								...
								while ($row = $res->fetch(PDO::FETCH_OBJ)) {
									...
									if($row->id_curso == 1000000391){
										$primeiro_nome = explode(" ", $row->nome);
										$primeiro_nome = utf8_encode($primeiro_nome[0]);
										$texto = urlencode("Bom dia $primeiro_nome, tudo bem?\nSou o Fernando, coordenador do Curso de Engenharia de Software da FAG.\nVocê esteve na nossa Feira das Profissões no dia 03, e informou no aplicativo o curso de Engenharia de Software com curso pretendido.\nJá fez sua inscrição para o Vestibular?");
									}

								?>
									<tr>
										<td><?= $i ?></td>
										<td width="10"><b><?= utf8_encode($row->cursoPretendido) ?></b></td>
										<td><?= utf8_encode($row->nome) ?></td>
										<td><?= $row->email ?></td>
										<td><a target="_blank" href="https://api.whatsapp.com/send?phone=55<?=$telefone?>&text=<?=$texto?>"><?= $row->telefone ?></a></td>
										<td><?= $row->ds_cortesia?></td>
										<td><?= $row->id_pessoa ? "SIM" : "-" ?></td>
							...
							<tbody>
								...
								$sql = "SELECT
										...
										FROM
											...
											INNER JOIN xp.cursos c ON a.cursoPretendido = c.curso
										WHERE
											colegio <> '' 
											and id_curso IN ( $cursos ) 
										GROUP BY
											colegio,
											cidade 
										ORDER BY
