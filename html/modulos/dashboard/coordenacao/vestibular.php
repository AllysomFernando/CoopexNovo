...
$id_campus = $_SESSION['coopex']['usuario']['id_faculdade'] == 1000000002 ? 1 : 2;
...
$sql = "SELECT
		...
		WHERE
			pagamento = 1 AND id_curso IN ($cursos)";
$res = $coopex_antigo->query($sql);
$row = $res->fetch(PDO::FETCH_OBJ);
...
$sql = "SELECT
		...
		$vestibular.inscricao
			INNER JOIN $vestibular.pessoa USING ( id_pessoa ) 
		WHERE
			pagamento = 0  AND id_curso IN ($cursos)
		AND
			id_ensino_medio = 3";
$res = $coopex_antigo->query($sql);
$row = $res->fetch(PDO::FETCH_OBJ);
...
<script>
	...
	$sql = "SELECT
			...
			date(dt_inscricao) as data_inscricao
		FROM
			$vestibular.inscricao 
			WHERE id_curso IN ($cursos)
		GROUP BY
	...
	$sql = "SELECT
			...
			WHERE
				pagamento = 1 
				AND id_cortesia = 0 AND id_curso IN ($cursos)";
	$res = $coopex_antigo->query($sql);
	$row = $res->fetch(PDO::FETCH_OBJ);
	$qtd_boleto = $row->qtd;
