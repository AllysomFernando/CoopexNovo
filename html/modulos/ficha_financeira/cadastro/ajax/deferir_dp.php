<?php
	...
	$sql = "INSERT INTO `ficha_financeira`.`ficha_financeira_etapa` ( `id_ficha_financeira`, `id_etapa`, `data_cadastro`, `enviado_por` )
			VALUES ($id_registro, 15, now(), '$usuario')";		

	$res = $coopex->query($sql);

	$sql = "UPDATE ficha_financeira.ficha_financeira SET id_etapa=15 WHERE (id_ficha_financeira=$id_registro)";
	$coopex->query($sql);

	$remetente		= "fichafinanceira@fag.edu.br";
	$destinatario 	= "tesouraria@fag.edu.br";

	$assunto = "Ficha Financeira - Desconto DP Deferido: $id_registro";
