<?php session_start();
	...
	//SE FOR O CURSO DE BIOLÓGICAS CARREGA AS DISICPLINAS DO BACHARELADO E LICENCIATURA
	$where_curso = $id_curso == 1000000058 || $id_curso == 1000000059 ? "AND (crs_id_curso = 1000000058 OR crs_id_curso = 1000000059)" : "AND crs_id_curso = $id_curso";
	...
	$sql = "SELECT DISTINCT
			...
			FROM
				...
				INNER JOIN academico..GCR_grade_curricular gcr ON gcr.gcr_id_atividade = atc.atc_id_atividade
				INNER JOIN academico..CRR_curriculo crr ON crr.crr_id_curriculo = gcr.gcr_id_curriculo
				INNER JOIN academico..CRS_curso crs ON crs.crs_id_curso = crr.crr_id_curso 
			WHERE
				 atc_id_atividade = gcr_id_atividade 
