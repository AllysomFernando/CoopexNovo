	...
	if($id_turma){
		$sql = "SELECT TOP
				...
				FROM
					...
					LEFT JOIN academico..crs_curso ON crs_id_curso = rca_id_curso
					LEFT JOIN academico..OCP_ocorrencia_por_periodo ON ocp_id_registro_curso = rca_id_registro_curso
					LEFT JOIN academico..OCA_ocorrencia_academica ON oca_id_ocorrencia = ocp_id_ocorrencia_corrente
					LEFT JOIN academico..ALU_aluno ON alu_id_pessoa = rca_id_aluno
					LEFT JOIN academico..PEL_periodo_letivo ON pel_id_periodo_letivo = ocp_id_periodo_letivo
					LEFT JOIN academico..PAC_pacote ON pac_id_turma_curso = rca_id_turma_curso 
					LEFT JOIN academico..HIS_historico_ingresso_saida ON his_id_registro_curso = rca_id_registro_curso
				WHERE
	...
	if(mssql_num_rows($res) > 0){
	 	while($row = mssql_fetch_assoc($res)){
			...
			if($row['atc_id_unidade_responsavel'] == 1000000080){
				...
				$row['atc_nm_atividade'] = (trim($row['atc_nm_atividade'])) ."<br><small>". trim($row2['atc_nm_atividade'])."</small>";
				$row['dp'] = $dp;
				...
				$row['atc_nm_atividade'] = (trim($row['atc_nm_atividade']));
			...
			$row['classe_padrao'] = get_classe($id_semestre, $id_disciplina, $id_pessoa);
