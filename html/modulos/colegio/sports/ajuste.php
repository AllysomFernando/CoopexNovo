...
while ($row = $res->fetch(PDO::FETCH_OBJ)) {
	...
	$sql2 = "SELECT
			...
			WHERE
				...
				AND PES_ID_PESSOA = $row->id_pessoa";
	...
	$sql3 = "UPDATE `colegio`.`sports` SET `id_curso` = $row2->crs_id_curso, `id_turno` = '$row2->turno', `id_serie` = '$row2->ser_id_serie' WHERE `id_pessoa` = $row->id_pessoa";
