			...
			while($row_grupo = $res_grupo->fetch(PDO::FETCH_OBJ)){
				...
				if($horario->id_grupo_aluno == 1){ //TURMA INTEIRA
					...
					$sql = "INSERT INTO `medicina`.`sub_grupo`(`id_grupo`, `id_horario`, `alunos`, `nome`)
									VALUES ($row_grupo->id_grupo, $horario->id_horario, $grupo_periodo->alunos_turma, '$nome')";
					$coopex->query($sql);
					...
					$total_alunos_subgrupo = ceil($grupo_periodo->alunos_turma / 2);
					...
					for($i=1; $i<=$qtd_sub_grupos; $i++){ 
						...
						$sql = "INSERT INTO `medicina`.`sub_grupo`(`id_grupo`, `id_horario`, `alunos`, `nome`)
										VALUES ($row_grupo->id_grupo, $horario->id_horario, $grupo_periodo->alunos_grupo, '$nome')";
						$coopex->query($sql);
					}
