							...
							<div class="panel-content">
													...
													if(isset($dados->id_professor)){
														...
														$sql = "SELECT 
																...
																FROM
																	medicina.professor 
																WHERE
																	id_professor IN ($id_docente)";
								

														$res = $coopex->query($sql);
														while($row = $res->fetch(PDO::FETCH_OBJ)){
												?>
														<option  value="<?php echo $row->id_professor?>"><?php echo trim(utf8_encode($row->nome))?></option>
												<?php
												 		}
												?>
												<?php	
								...
								<div class="form-row">
									<div class="col-md-12 mb-3">
										<label class="form-label" for="validationCustom02">Observação</label>
										<textarea type="text" class="form-control" name="obs" placeholder="" ><?php echo isset($dados->obs) ? utf8_encode($dados->obs) : ""?></textarea>
									</div>
								</div>
...
<script>
	...
	$(document).ready(function(){
		...
		$(".js-consultar-usuario-medicina").select2({
			ajax:{
				url: "modulos/_core/buscar_usuario_medicina.php",
				dataType: 'json',
				delay: 250,
				data: function(params){
					return {
