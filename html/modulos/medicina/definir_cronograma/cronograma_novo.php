...
<?php
	...
	/* OK - AULAS TEÓRICAS, NÃO NECESSITAM DE VERIFICAÇÃO DE CHOQUE DE HORÁRIO */
	function turma_inteira($id_horario){
		
		$coopex = new PDO("mysql:dbname=coopex;host=10.0.0.41", 'fernando', 'indioveio');
		
		$sql = "SELECT
					horario_data.*, grupo_aluno,
					qtd_alunos
				FROM
	...
	function meio_grupo($id_horario){
		...
		//print_r($arr_horario_data);

		foreach ($arr_sub_grupo as $key => $value) {
			print_r($value);
			echo "<br>";
		}
