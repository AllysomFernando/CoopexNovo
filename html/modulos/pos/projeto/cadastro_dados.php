...
if ($_POST) {
	...
	if (isset($excluir_registro)) {
		...
		$ids_disciplinas = isset($_POST['disciplinas']) ? $_POST['disciplinas'] : "";
		...
		try {
			...
			$sql_limpar_disciplinas = "UPDATE pos.estrutura_curricular SET id_projeto = null WHERE id_projeto = $id_registro OR id_projeto IS NULL";
			echo "<br> REMOVIDO -> " . $sql_limpar_disciplinas;
			$coopex->query($sql_limpar_disciplinas);

			if ($ids_disciplinas) {
				foreach ($ids_disciplinas as $disciplina) {
					$sql_adicionar_disciplinas = "UPDATE pos.estrutura_curricular SET id_projeto = $id_registro WHERE id_disciplina = $disciplina";
					$stm = $coopex->prepare($sql_adicionar_disciplinas);
					$stm->execute();
					echo "<br> ADICIONADO -> " . $sql_adicionar_disciplinas;
				}
			}
