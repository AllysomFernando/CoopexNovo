<?php
	...
	if(isset($_GET['id'])){
		...
		if($pre_matriculado){
			...
			$tempo = [];
			$sql = "SELECT *, count( * ) as dia FROM
						(SELECT id_reoferta, id_pessoa, data_reoferta, horario_inicio, horario_termino
						FROM coopex_reoferta.cronograma INNER JOIN coopex_reoferta.matricula m USING ( id_reoferta ) 
						WHERE id_pessoa = $id_pessoa UNION SELECT id_reoferta, NULL, data_reoferta, horario_inicio, horario_termino 
						FROM coopex_reoferta.cronograma WHERE id_reoferta = ".$_GET['id'].") AS choque GROUP BY data_reoferta HAVING dia > 1";
			$res = $coopex->query($sql);
			if($res->rowCount()){
				while($choque = $res->fetch(PDO::FETCH_OBJ)){
					if($choque->dia > 1){

						$sql = "SELECT * FROM coopex_reoferta.cronograma WHERE id_reoferta = ".$_GET['id']." AND data_reoferta = '".$choque->data_reoferta."'";
						...
						$sql = "SELECT * FROM
									...
									FROM coopex_reoferta.cronograma INNER JOIN coopex_reoferta.matricula m USING ( id_reoferta ) 
									WHERE id_pessoa = $id_pessoa UNION SELECT id_reoferta, NULL, data_reoferta, horario_inicio, horario_termino 
									FROM coopex_reoferta.cronograma WHERE id_reoferta = ".$_GET['id']."  ) AS choque WHERE data_reoferta = '".$choque->data_reoferta."' AND id_pessoa is not null";
						$res2 = $coopex->query($sql);
						while($choque2 = $res2->fetch(PDO::FETCH_OBJ)){

							/*echo "--------------------------------------------------<br>";
							echo '$data_reoferta->horario_inicio -> '.$data_reoferta->horario_inicio."<br>";
							echo '$choque2->horario_inicio -> '.$choque2->horario_inicio."<br>";
							echo '$data_reoferta->horario_termino -> '.$data_reoferta->horario_termino."<br>";
							echo '$choque2->horario_termino -> '.$choque2->horario_termino."<br><strong>";*/
							
							//SE A HORA DE INÍCIO FOR MAIOR QUE A HORA DO CHOQUE E HORA DE SAIDA FOR MENOR QUE HORA DE SAIDA DO CHOQUE
							if((@strtotime($data_reoferta->horario_inicio) >= @strtotime($choque2->horario_termino))){
								
							} else if((@strtotime($data_reoferta->horario_termino) <= @strtotime($choque2->horario_inicio))){
								
							} else if((@strtotime($data_reoferta->horario_inicio) >= @strtotime($choque2->horario_inicio)) && 
							  (@strtotime($data_reoferta->horario_termino) <= @strtotime($choque2->horario_termino))){
								...
								$total = @gmdate('H:i:s', @strtotime( $data_reoferta->horario_termino ) - @strtotime($choque2->horario_inicio ));
								$tempo[] = $total;
							} else if((@strtotime($data_reoferta->horario_inicio) <= strtotime(@$choque2->horario_inicio)) && 
							  		 (@strtotime($data_reoferta->horario_termino) >= strtotime(@$choque2->horario_termino))){
								
								$total = @gmdate('H:i:s', @strtotime( $choque2->horario_termino ) - @strtotime($choque2->horario_inicio ));
								$tempo[] = $total;
							} else if((@strtotime($data_reoferta->horario_inicio) >= @strtotime($choque2->horario_inicio)) && 
							  		 (@strtotime($data_reoferta->horario_termino) >= @strtotime($choque2->horario_termino))){
								
								$total = @gmdate('H:i:s', strtotime( $choque2->horario_termino ) - strtotime($data_reoferta->horario_inicio ));
								$tempo[] = $total;
							} 
							//echo "</strong><br>--------------------------------------------------fim<br>";
						}			
					}
				}
				...
				echo $choque_de_horario_tempo = $segundos * 100 / ($reoferta->carga_horaria * 60 * 60);
