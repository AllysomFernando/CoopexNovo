...
//		print_r($parametros_json);
	...
	if (!empty($retorno->cdErro)) {
		throw new Exception(die('<pre>Nao foi possível registrar o boleto. --> ' . $retorno->msgErro));
	}
...
function registrar_bradesco_sports($nome, $cpf, $endereco, $bairro, $cidade, $uf, $cep, $data_vencimento, $data_emissao, $valor_nominal, $seu_numero, $endereco_numero)
{
				...
				if ($res_cronograma_matricula->rowCount()) {
					...
					while ($cronograma = $res_cronograma->fetch(PDO::FETCH_OBJ)) {
						...
						if ((strtotime(@$data_reoferta->horario_inicio) >= strtotime(@$cronograma->horario_termino))) {
							...
							(strtotime($data_reoferta->horario_inicio) >= strtotime($cronograma->horario_inicio)) &&
